...
- If either the ancilla error rate ($\epsilon_{anc}$) or the \epsilon_{RO} are bigger than $\epsilon_{phys}$
...
- What is the difference between qubits used for quantum memory and quantum computation? In our case we consider both at the same time, isn't it?
...
- Does the $R_{dep}$ parameter mean that the polarizing model is included?
...
- Is this error model the one that they use in quantumsim?
